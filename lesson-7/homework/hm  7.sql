...
--task 4
select count(distinct category) as Unique_P from Products
--task 5
select sum(SaleAmount) as TotalSales from Sales
where ProductID = 7
--task 6 
select avg(age) as AveAge from Employees
--task 7 
select DepartmentName, count(*) as Tota_emp from Employees
...
group by CustomerID
--task 10 
select DepartmentName, COUNT(*) as EmpAmount from Employees
group by DepartmentName
having count(*)>5

--MEDIUM LEVEL 
--TASK 11
SELECT p.Category, SUM(s.SaleAmount) AS TotalSales, AVG(s.SaleAmount) AS AvgSales FROM Sales s
JOIN Products p 
ON s.ProductID = p.ProductID
GROUP BY p.Category;
--TASK 12 
SELECT COUNT(e.EmployeeID) AS TotalHR FROM Employees e
JOIN Departments d 
...
GROUP BY DepartmentName
--TASK 15
SELECT DepartmentName, AVG(SALARY) AS AVGSALARY, COUNT(*) AS EMPLOYESS FROM Employees
GROUP BY DepartmentName
--TASK 16
SELECT p.Category, AVG(p.Price) AS AvgPrice FROM Products p
GROUP BY p.Category
HAVING AVG(p.Price) > 400;
--task 17
select year(SaleDate) as SalesDATE, SUM(SaleAmount)  AS SalesAmount from Sales
group by year(SaleDate)
--task 18
SELECT c.CustomerID, COUNT(s.SaleID) AS NumOrders FROM Sales s
JOIN Customers c 
ON s.CustomerID = c.CustomerID
GROUP BY c.CustomerID
HAVING COUNT(s.SaleID) >= 3;
-- task 19
SELECT DepartmentName, AVG(e.Salary) AS AvgSalary
FROM Employees e
GROUP BY DepartmentName
...
--TASK 21
SELECT c.CustomerID, sum(S.SaleAmount) as TOTALSALES FROM SALES S 
JOIN Customers c
on c.CustomerID=s.CustomerID
group by c.CustomerID
having sum(s.saleamount) >1500
--task 22
select DepartmentName, sum(salary) as totalsalary, avg(salary) as abgsalary from Employees 
group by DepartmentName
...
select year(o.OrderDate) as orderyear, min(o.quantity) as MinQuantity, max(o.Quantity) as maxquantity from Orders o
join Products p
