...
--TASK 3
SELECT CustomerID, FirstName, City FROM Customers
WHERE City LIKE 'L%'
--TASK 4
...
--MEDIUM LEVEL 
--TASK  10
SELECT O.CustomerID, O.OrderDate, I.InvoiceDate FROM Orders O 
JOIN Invoices I
ON O.CustomerID=I.CustomerID
WHERE YEAR(O.OrderDate)=2023 AND MONTH(O.OrderDate)=1 AND I.InvoiceDate IS NULL
--TASK 11
...
SELECT ProductName FROM Products_Discounted
--TASK 13
SELECT YEAR(ORDERDATE) AS SaleDates, AVG(TotalAmount) as totalamount FROM Orders
...
--HARD LEVEL 
--TASK 16 
SELECT TOP 3 CustomerID, SUM(TotalAmount) AS TotalSpend FROM Invoices
GROUP BY CustomerID
...
select p.ProductName, count(s.saleid) as frequency from Sales as s 
join Products as p 
