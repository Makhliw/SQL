...
--TASK 2
SELECT c.FirstName, C.LastName, o.OrderDate FROM ORDERS O 
JOIN Customers C 
...
--TASK 4
 ...
 --TASK 5 
 SELECT O.OrderID, O.OrderDate, P.PaymentDatE, P.Amount FROM Payments P 
 ...
 --MEDIUM LEVEL
 --TASK 10
SELECT D.DepartmentName, COUNT(E.EMPLOYEEID) AS EMPCOUNT FROM Employees E 
JOIN Departments D 
ON D.DepartmentID=E.DepartmentID
...
--TASK 12 
SELECT C.FirstName, C.LastName, COUNT(O.OrderID) AS ORDEAMOUNT FROM Customers C
JOIN Orders O
ON C.CustomerID=O.CustomerID
...
AS DAVG 
ON E.DepartmentID = DAVG.DepartmentID
...
--task 22
SELECT e1.Name AS Employee1, e2.Name AS Employee2, e1.ManagerID, e1.Salary FROM Employees e1
JOIN Employees e2 
